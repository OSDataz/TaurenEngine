	...
	public static class Filex
	{
		...
		public static bool Copy(string sourcePath, string destPath, bool isFull = false)
		{
			...
			if (sourceIsFile)
			{
				...
				{
					// 文件 到 文件夹
					File.Copy(sourcePath, $"{Pathx.FormatEnd(destPath, true)}{Path.GetFileName(sourcePath)}", true);
				}
			}
			else
			{
				// 文件夹 到 文件夹
				sourcePath = Pathx.FormatEnd(sourcePath, false);
				destPath = Pathx.FormatEnd(destPath, false);
