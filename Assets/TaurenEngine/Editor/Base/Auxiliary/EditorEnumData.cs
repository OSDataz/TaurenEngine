...
namespace TaurenEditor
{
    ...
    {
		#region 静态管理
		private static Dictionary<Type, EditorEnumData> _dictionary;

		public static EditorEnumData Get<T>() where T : Enum
		{
			_dictionary ??= new Dictionary<Type, EditorEnumData>();
			if (!_dictionary.TryGetValue(typeof(T), out var enumPopup))
			{
				enumPopup = new EditorEnumData();
				enumPopup.Init<T>();
