...
namespace TaurenEditor.ModConfig
{
	...
	{{{1}
		...
		private const string ClassTemplate = @"	public class {0} : TaurenEngine.ModConfig.IJonsConfig
	{{{1}
		...
		{{
			...
			{{{3}
				...
				TaurenEngine.Core.Log.Error($""配置表类型错误：{{jsonData.GetType()}}->{{typeof(Data)}}"");
		}}{4}
	}}";
		#endregion
		...
		private void GenerateConfigScript(DataTable sheet, string savePath, string scriptNamespace, Encoding encoding)
		{
			...
			for (int i = 2; i < len; ++i)
			{
				...
				if (note[0] == Key_Quick)
					builder4.Append($"\n		public static {className} {note.Split(Key_Split)[0].Substring(1)} => TaurenEngine.ModConfig.ConfigData.Instance.GetList<{className}>()[{index}];");
		...
		private string PreloadTemplate = @"	public class ConfigPreload : TaurenEngine.ModConfig.ConfigPreloadBase
	{{
