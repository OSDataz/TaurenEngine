...
namespace TaurenEditor.Common
{
	...
	public class BatScript
	{
		...
		public void Run()
		{
			...
			var path = $"{EditorHelper.ProjectConfigPath}/Temp/BatOrder.bat";

			// 保存代码文件
			Filec.SaveText(path, text);
		...
		{
			fromPath = Pathx.FormatEnd(fromPath, false);
			toPath = Pathx.FormatEnd(toPath, false); ;

			if (string.IsNullOrEmpty(fromPath) || string.IsNullOrEmpty(toPath))
				return;

			if (!Filec.Exists(fromPath, out var fromIsFile))
				return;// 原文件不存在

			var toIsExist = Filec.Exists(toPath, out var toIsFile);
			...
			if (fromIsFile)
			{
				...
				{
					if (toIsFile)
						Filec.CreateDirectoryByFilePath(toPath);
					else
						Filec.CreateDirectoryByDirectoryPath(toPath);
				}

				if (!toIsFile)
					toPath = $"{Pathx.FormatEnd(toPath, true)}{Path.GetFileName(fromPath)}";
			...
			else
			{
				...
				Filec.CreateDirectoryByDirectoryPath(path);
