...
namespace Tauren.Framework.Runtime
{
	...
	/// </summary>
	internal class TimerList : DObject, IRecycle
	{
		/// <summary>
		/// 循环状态
		/// </summary>
		...
		public Timer Find(Predicate<Timer> match)
		{
			...
			for (int i = 0; i < len; ++i)
			{
				if (status[i] != Status.Remove)
				{
					var timer = refrenceList.Get<Timer>(i);
					if (match.Invoke(timer))
						return timer;
				}
			}
		...
		public void ForEach(Action<Timer> action, bool resetLoop = false)
		{
			...
			for (i = 0; i < len; ++i)
			{
				if (status[i] == Status.None)
				{
					...
					{
#endif
						action.Invoke(refrenceList.Get<Timer>(i));// todo 需考虑使用try catch的必要性
#if BUILD_MODE_DEBUG
					}	
