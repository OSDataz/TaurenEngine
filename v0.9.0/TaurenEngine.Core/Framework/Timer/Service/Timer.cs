...
namespace TaurenEngine
{
	...
	public class Timer : PoolObject<Timer>
	{
		...
		public void Stop()
		{
			...
			if (type == TimerType.Update || type == TimerType.UpdateInterval)
				ITimerService.Instance.UpdateList.Remove(this);
			else if (type == TimerType.LateUpdate || type == TimerType.LateUpdateInterval)
				ITimerService.Instance.LateUpdateList.Remove(this);
			else if (type == TimerType.FixedUpdate || type == TimerType.FixedUpdateInterval)
				ITimerService.Instance.FixedUpdateList.Remove(this);
			else
				Debug.LogError($"Timer类型异常：{type}");
		}
