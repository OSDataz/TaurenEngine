	...
	public static class ILRuntimeHelper
	{
		...
		public static void GenerateCLRBindingByAnalysis(ILRuntimeData data)
		{
			...
			foreach (var dll in dlls)
			{
				...
				using (FileStream fs = new FileStream($"{EditorHelper.ProjectPath}/Library/ScriptAssemblies/{dll.name}", FileMode.Open, FileAccess.Read))
				{
		...
		private static void GenerateAdpatorRegisterCode(ILRuntimeAdaptorGroupEo groupEo)
		{
			...
			var files = Directory.GetFiles($"{EditorHelper.ProjectPath}/{groupEo.AdaptorSavePath.Value}", "*.cs");
